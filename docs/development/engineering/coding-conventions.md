...
- When maintaining the baseline demos, preserve their `ui_pixel` theme (sky background, grass, title plate, mascot, ink-outlined panels) unless a UI change is requested. This is not an application contract: newly developed applications must design their own UI as required by the [AI guide](../ai-guide.md), not inherit the demo screens or require `ui_pixel_screen_create()` / `ui_pixel_panel_create()`.
