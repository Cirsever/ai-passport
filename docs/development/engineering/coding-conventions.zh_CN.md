...
- **基线主题与新应用分开**：维护基线 demo 时，除非用户要求修改 UI，否则保留其 `ui_pixel` 主题（天空底色、草地、标题牌、吉祥物、墨色描边面板）。这不是应用契约：新开发应用必须按 [AI 指南](../ai-guide.zh_CN.md)设计独立 UI，不能直接继承 demo 页面，也不强制使用 `ui_pixel_screen_create()` / `ui_pixel_panel_create()`。
